...
public class DragGridView extends GridView {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mDragPosition == AdapterView.INVALID_POSITION
                        || mDragPosition == (((DragAdapter) mDragAdapter).isOk ? ((DragAdapter) mDragAdapter).getCount()
                        : ((DragAdapter) mDragAdapter).getCount() - 1)) {
    ...
    private Runnable mScrollRunnable = new Runnable() {
        ...
        public void run() {
            int scrollY;
            if (getFirstVisiblePosition() == 0 || isOk()) {
                mHandler.removeCallbacks(mScrollRunnable);
            }
    ...
    private boolean isOk() {
        return ((DragAdapter) mDragAdapter).isOk ? getLastVisiblePosition() == getCount() - 1 : getLastVisiblePosition() == getCount();
    }
    ...
    private void onSwapItem(int moveX, int moveY) {
        ...
        if (tempPosition != mDragPosition && tempPosition != AdapterView.INVALID_POSITION && mAnimationEnd) {
            if (((DragAdapter) mDragAdapter).isOk) {
                if (tempPosition != ((DragAdapter) mDragAdapter).getCount()) {
                    startChange(tempPosition);
                }

            } else {
                if (tempPosition != ((DragAdapter) mDragAdapter).getCount() - 1) {
                    startChange(tempPosition);
                }
            }
        }
    ...
    private void startChange(final int tempPosition) {
