...
public class DateTimePickDialogUtil implements OnDateChangedListener, OnTimeChangedListener {
	...
	public void init(DatePicker datePicker, TimePicker timePicker) {
		try {
			...
			if (!TextUtils.isEmpty(initDateTime)) {
				...
				initDateTime = calendar.get(Calendar.YEAR) + "年" + (calendar.get(Calendar.MONTH) + 1) + "月" + calendar.get(Calendar.DAY_OF_MONTH)
						+ "日 ";
				calendar = getCalendarByInintData(initDateTime);
			}
	...
	public AlertDialog dateTimePicKDialog(final EditText inputDate) {
		return dateTimePicKDialog(inputDate, false, false);
	}

	@SuppressLint("NewApi")
	public AlertDialog dateTimePicKDialog(final EditText inputDate, boolean setMaxDate, boolean setMinDate) {
	...
	@SuppressLint("NewApi")
	public AlertDialog dateTimePicKDialog(OnClickListener ok, OnClickListener cancle, boolean setMaxDate, boolean setMinDate) {
		LinearLayout dateTimeLayout = (LinearLayout) activity.getLayoutInflater().inflate(R.layout.common_datetime, null);
		datePicker = (DatePicker) dateTimeLayout.findViewById(R.id.datepicker);
		if (setMinDate) {
			datePicker.setMinDate(System.currentTimeMillis() - 1000);
		...
		ad = new AlertDialog.Builder(activity).setTitle(initDateTime).setView(dateTimeLayout).setPositiveButton("设置", ok)
