package com.wei.view.customer.view.cycleViewPager;
 ...
 */
@SuppressLint("NewApi")
public class CycleViewPager extends LinearLayout implements OnPageChangeListener, View.OnTouchListener {
    ...
    public CycleViewPager(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    public View onCreateView() {
        view = LayoutInflater.from(context).inflate(R.layout.view_cycle_viewpager_contet, null);
        ...
        handler = new CycleViewPagerHandler(context) {
            ...
            public void handleMessage(Message msg) {
                ...
                if (msg.what == WHEEL && imageViews.size() > 1) {
                    if (!isScrolling) {
                ...
                if (msg.what == WHEEL_WAIT && imageViews.size() > 1) {
                    handler.removeCallbacks(runnable);
                    handler.postDelayed(runnable, time);
                }
            }
    ...
    public void setData(List<ImageView> views, List<String> list, ImageCycleViewListener listener, int showPosition) {
        ...
        for (int i = 0; i < indicators.length; i++) {
            View view = View.inflate(context, R.layout.view_cycle_viewpager_indicator, null);
    ...
    public void setWheel(boolean isWheel) {
        ...
        isCycle = true;
        if (getScrollable() && isWheel && infos != null && infos.size() > 1) {
            handler.postDelayed(runnable, time);
        }
    }
    ...
    final Runnable runnable = new Runnable() {
        ...
        @Override
        public void run() {
            if (context != null && !((Activity) context).isFinishing() && isWheel && infos != null && infos.size() > 1) {
...
//        }
    ...
    public boolean getScrollable() {
        return viewPager.getScrollable();
    }
