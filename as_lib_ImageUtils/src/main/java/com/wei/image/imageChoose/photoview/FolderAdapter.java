package com.wei.image.imageChoose.photoview;
...
import com.wei.image.imageChoose.BitmapCache;
import com.wei.image.imageChoose.ImageItem;
import com.wei.image.imageChoose.ui.AlbumActivity;
import com.wei.image.imageChoose.ui.ShowAllPhoto;
...
public class FolderAdapter extends BaseAdapter {
	...
	BitmapCache.ImageCallback callback = new BitmapCache.ImageCallback() {
		...
		public void imageLoad(ImageView imageView, Bitmap bitmap, Object... params) {
			if (imageView != null && bitmap != null) {
				...
				String tag2 = (String) imageView.getTag();
				if (url != null && url.equals(tag2)) {
					int degree = readPictureDegree(tag2);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (convertView == null) {
			convertView = View.inflate(context, R.layout.activity_plugin_camera_select_folder, null);
		...
		if (AlbumActivity.contentList.get(position).imageList != null) {
			...
			// 封面图片路径
			path = AlbumActivity.contentList.get(position).imageList.get(0).getImagePath();
		...
		if (path.contains("android_hybrid_camera_default"))
			holder.imageView.setImageResource(R.drawable.moren);
		else {
			...
			cache.displayBmp(holder.imageView, item.getThumbnailPath(), item.getImagePath(), callback);
		}
		// 为封面添加监听
	...
	private class ImageViewClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			intent.setClass(context, ShowAllPhoto.class);
