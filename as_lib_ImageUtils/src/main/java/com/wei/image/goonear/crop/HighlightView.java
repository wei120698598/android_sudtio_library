...
package com.wei.image.goonear.crop;
...
class HighlightView {
	...
	protected void draw(Canvas canvas) {
		...
		if (!hasFocus()) {
			...
			Paint paint = hasFocus() ? mFocusPaint : mNoFocusPaint;
			if (mCircle) {

				float width = mDrawRect.width();
				float height = mDrawRect.height();
//				canvas.drawCircle(mDrawRect.left + (width / 2), mDrawRect.top + (height / 2), width / 2, paint);
				bitmap = Bitmap.createBitmap(mContext.getWidth(), mContext.getHeight(), Config.ARGB_8888);
				
				

				Canvas tempcCanvas = new Canvas(bitmap);
				tempcCanvas.drawColor(paint.getColor());
				tempcCanvas.drawCircle(mDrawRect.left + (width / 2), mDrawRect.top + (height / 2), width / 2, paint2);
				...
				mOutlinePaint.setColor(0xFF4682B4);

				canvas.drawRect(0, mDrawRect.top, mDrawRect.left, mDrawRect.bottom, paint);
				canvas.drawRect(0, 0, mContext.getWidth(), mDrawRect.top, paint);
				canvas.drawRect(mDrawRect.right, mDrawRect.top, mContext.getWidth(), mDrawRect.bottom, paint);
				canvas.drawRect(0, mDrawRect.bottom, mContext.getWidth(), mContext.getHeight(), paint);
			}
